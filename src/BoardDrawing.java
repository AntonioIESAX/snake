...
public class BoardDrawing extends JPanel {
        ...
        for (Rectangle cell : cells) {
            ...
            for (int pl = 0; pl < bs.maxPlayers; pl++) {
                if (bs.players.get(pl).getPosition() == cellnos[i]) {
                    DrawPlayer(g2d, pl, cell, cellWidth, cellHeight);
                }
            }
            ...
            if (cellnos[i] == row * col - 1) {
                for (int pl = 0; pl < bs.maxPlayers; pl++) {
                    if (bs.players.get(pl).getPosition() >= cellnos[i]) {
                        DrawPlayer(g2d, pl, cell, cellWidth, cellHeight); //only one player considered here
    ...
    /**
     * @param g2d tipo Graphics2D
     */    
    private void DrawingSnakes(Graphics2D g2d) {
